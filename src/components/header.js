...
const NavLink = ({ children, ...props }) => (
  <Link
    {...props}
    className='inline-block py-2 px-3 border-b-2 border-zinc-100 dark:border-zinc-800 hover:border-zinc-200 hover:dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-700'
    activeClassName='border-b-2 border-b-rose-700 dark:border-b-rose-500 hover:border-b-rose-700 hover:dark:border-b-rose-500'
  >{children}</Link>
)
    ...
    <header className="flex items-center border-b border-b-neutral-400 dark:border-b-neutral-600 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 font-semibold">
      ...
      <div className="group flex flex-col" id="mobile-menu">
        ...
        <nav className="md:hidden max-h-0 overflow-hidden peer-checked:max-h-full" style={{ transition: "max-height .2s ease-out" }}>
          <ul className="mb-0">
            {navLinks.map(([title, url]) => (
              <li className="block" key={title}><NavLink to={url}>{title}</NavLink></li>
            ))}
          </ul>
        </nav>
      </div>
      <button className="ml-auto mr-3 self-start py-2 border-b-2 border-b-transparent" onClick={() => setIsDark(isDark === "dark" ? "light" : "dark")}>
        {isDark === "dark" ?
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-zinc-300">
            <path strokeLinecap="round" strokeLinejoin="round" d="M21.752 15.002A9.72 9.72 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998Z" />
          </svg>
          : <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6 text-zinc-600">
